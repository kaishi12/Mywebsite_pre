...
namespace MyWebsite.Service.Common
{
    public class DateDifference
    {
       ...
       readonly private int[] monthDay = new int[12] { 31, -1, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

        // ngày bắt đầu
       readonly private DateTime fromDate;

        // ngày kết thúc
        readonly private DateTime toDate;
        ...
        public DateDifference(DateTime d1, DateTime d2)
        {
            if (d1 > d2)
            {
                ...
                this.toDate = d1;
                TimeSpan a = d1 - d2;
                this.hour = int.Parse(a.ToString());
                DateDiff();
            }
            else
            {
